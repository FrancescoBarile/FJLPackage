---
title: "StepeestDescend_R_vs_Cpp"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{StepeestDescend_R_vs_Cpp}
...
#The aim of this Monte Carlo experiment is to show that the estimate of a Classical Linear Regression model (i.e. when all the classical assumptions of the linear regression model are satisfied) is consistent.
  ...
  SD_r_fun = {
    ...
    for(i in 1:length(n)){
      for(j in 1:nsim){
        ...
        beta_hat[,j] = betahat_SD_R(beta, x, y, tolerance)
      }
  ...
  SD_cpp_fun = {
    ...
    for(i in 1:length(n)){
      for(j in 1:nsim){
        ...
        x = cbind(rep(1,n[i]), x1, x2, x3)
        y = as.numeric( x%*%beta0 + rnorm(n[i]) )
        beta_hat[,j] = betahat_SD_Cpp(beta, x, y, tolerance)
      }
